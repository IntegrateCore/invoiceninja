...
<script>
    ...
    const config = {
        ...
        text: "{{ ($account ?? false) && !$account->isPaid() ? config('ninja.brand_name') : (isset($company) && !is_null($company) ? $company->name : config('ninja.brand_name')) }} {{ ctrans('texts.nordigen_handler_subtitle', [], $lang ?? 'en') }}",
        // Logo URL that will be shown below the modal form.
        logoUrl: "{{ ($account ?? false) && !$account->isPaid() ? asset('images/integratecore-icon.png') : (isset($company) && !is_null($company) ? $company->present()->logo() : asset('images/integratecore-icon.png')) }}",
